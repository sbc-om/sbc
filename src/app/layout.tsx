...
export default async function RootLayout({
  ...
  const pwaSplashInit = `(function(){try{var r=document.documentElement;var standalone=(window.matchMedia&&window.matchMedia('(display-mode: standalone)').matches)||window.navigator.standalone===true;var shown=false;try{shown=sessionStorage.getItem('sbc-pwa-splash-shown')==='1'}catch(_){shown=false}if(!standalone||shown)return;r.dataset.pwaSplash='show';r.dataset.pwaSplashActive='true';r.style.backgroundColor='#ebf4ff';r.style.backgroundImage='radial-gradient(1200px 720px at 8% -10%, rgba(8,119,251,.35), transparent 62%),radial-gradient(900px 560px at 92% -12%, rgba(6,182,212,.26), transparent 58%),linear-gradient(150deg,#f8fbff 0%,#ebf4ff 44%,#f5fcff 100%)';var started=Date.now();var hidden=false;var hide=function(){if(hidden)return;hidden=true;var elapsed=Date.now()-started;var minVisible=1000;var finish=function(){r.dataset.pwaSplash='hide';delete r.dataset.pwaSplashActive;r.style.removeProperty('background-image');try{sessionStorage.setItem('sbc-pwa-splash-shown','1')}catch(_){}};if(elapsed<minVisible){setTimeout(finish,minVisible-elapsed)}else{finish()}};window.addEventListener('load',hide,{once:true});setTimeout(hide,3200)}catch(e){}})();`;

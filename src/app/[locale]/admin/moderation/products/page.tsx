...
import { listBusinessProductsModerationQueue } from "@/lib/db/businessContent";
import { isLocale, type Locale } from "@/lib/i18n/locales";

import { moderateBusinessProductAction } from "../actions";
...
export default async function AdminModerationProductsPage({
  ...
  const items = await listBusinessProductsModerationQueue(500);
  ...
  return (
    <AppPage>
      <div className="mb-6 flex items-center justify-between gap-3">
        <h1 className="text-2xl font-bold">{ar ? "مراجعة منتجات الأنشطة" : "Moderate Business Products"}</h1>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {filters.map((filter) => {
          ...
          const href = filter.value === "all"
            ? `/${locale}/admin/moderation/products`
            : `/${locale}/admin/moderation/products?status=${filter.value}`;
      ...
      {filteredItems.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredItems.map((item) => {
            const approve = moderateBusinessProductAction.bind(null, locale as Locale, item.id, "approved");
            const reject = moderateBusinessProductAction.bind(null, locale as Locale, item.id, "rejected");
              ...
              <article key={item.id} className="sbc-card rounded-2xl p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-medium">{item.price} {item.currency}</p>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <div className="rounded-xl border border-(--surface-border) p-3">
                    ...
                    <h3 className="text-sm font-semibold">{item.name.en}</h3>
                    {item.description?.en ? (
                      <div className="mt-2 text-sm leading-7">
                        <MarkdownRenderer content={item.description.en} />
                      </div>
                    ) : (
                      <p className="mt-2 text-xs text-(--muted-foreground)">{ar ? "بدون وصف" : "No description"}</p>
                    )}
                  </div>
                  <div className="rounded-xl border border-(--surface-border) p-3">
                    <p className="mb-2 text-xs font-semibold text-(--muted-foreground)">AR</p>
                    <h3 className="text-sm font-semibold">{item.name.ar}</h3>
                    {item.description?.ar ? (
                      <div className="mt-2 text-sm leading-7">
                        <MarkdownRenderer content={item.description.ar} />
                      </div>
                    ) : (
                      <p className="mt-2 text-xs text-(--muted-foreground)">{ar ? "بدون وصف" : "No description"}</p>
                    )}
